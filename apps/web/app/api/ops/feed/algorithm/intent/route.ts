/**
 * Intent Matrix API Route
 *
 * GET: Mevcut intent matrix konfigürasyonunu getir
 * PUT: Intent matrix konfigürasyonunu güncelle
 */
...
const CONFIG_TYPE = "intent_matrix";
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const intentTypes = ["meet_new", "activity_partner", "flirt", "serious_relationship"];
    const contentTypes = ["post", "mini_post", "poll", "voice_moment"];

    for (const intent of intentTypes) {
      if (!config[intent]) {
        return NextResponse.json({ error: `Missing intent type: ${intent}` }, { status: 400 });
      }
      for (const content of contentTypes) {
        if (typeof config[intent][content] !== "number") {
          return NextResponse.json(
            { error: `Invalid value for ${intent}.${content}` },
            { status: 400 }
          );
        }
      }
