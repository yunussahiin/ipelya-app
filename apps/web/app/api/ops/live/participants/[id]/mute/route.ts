/**
 * API: POST /api/ops/live/participants/[id]/mute
 * Katılımcının mikrofonunu kapatır (mute)
...
export async function POST(
  ...
  try {
    ...
    if (session?.livekit_room_name && session.status === 'live') {
      const result = await muteAllTracks(session.livekit_room_name, participant.user_id, true);
      if (!result.success) {
        console.error('LiveKit mute error');
      }
      mutedTracks = result.mutedTracks;
    }

    // Katılımcıyı muted olarak işaretle
    await adminSupabase
      .from('live_participants')
      .update({
        is_muted: true,
    ...
    await adminSupabase.from('live_admin_logs').insert({
      ...
      metadata: { 
        ...
        reason: reason || 'Admin tarafından susturuldu',
        muted_tracks: mutedTracks,
      },
    ...
    return NextResponse.json({
      success: true,
      message: 'Katılımcının mikrofonu kapatıldı',
      mutedTracks,
    });

  } catch (error) {
    console.error('Mute participant API error:', error);
    return NextResponse.json(
