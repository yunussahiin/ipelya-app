...
const AssistantModalButton = forwardRef<HTMLButtonElement, AssistantModalButtonProps>(
  ({ "data-state": state, ...rest }, ref) => {
    const tooltip = state === "open" ? "Asistanı Kapat" : "AI Asistan";
    ...
    return (
      <TooltipIconButton
        ...
        side="left"
        {...rest}
        className="aui-modal-button size-full rounded-full shadow-lg transition-transform hover:scale-110 active:scale-90 bg-primary text-primary-foreground"
        ref={ref}
      >
        <BotIcon
          data-state={state}
